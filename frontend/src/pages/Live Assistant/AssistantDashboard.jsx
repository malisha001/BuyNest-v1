  ...
  const handleAccept = async (request) => {
    try {
      ...
      const cartResponse = await axios.post(
        "http://localhost:4000/api/cart/getio",
        { userId }
      );
      ...
      if (cartResponse.data.success) {
        ...
        const acceptResponse = await axios.put(
          `http://localhost:4000/api/assist/${requestId}`,
          { accept: true }
        );
        ...
        if (acceptResponse.data.success) {
          ...
          console.error(
            "Failed to update the request accept status:",
  ...
  const handleRemove = async (requestId) => {
    try {
      ...
      if (response.data.success) {
        ...
        setSessionRequests(
          sessionRequests.filter((request) => request._id !== requestId)
        );
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4 lg:px-16">
      ...
      <header className="flex justify-between items-center mb-8 bg-gradient-to-r from-gray-100 via-white to-gray-200 p-6 rounded-t-2xl shadow-xl relative z-10">
        ...
        >
          <div
            className={`absolute top-1 left-1 w-6 h-6 rounded-full transition-transform transform bg-white ${
              availability ? "translate-x-8" : ""
            }`}
          >
            <FaPowerOff
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <section
            className={`rounded-2xl p-6 shadow-2xl hover:shadow-xl transition-shadow duration-300 ${
              availability ? "bg-gray-800" : "bg-white"
            }`}
          >
            <div className="flex justify-between items-center mb-4">
              <h2
                className={`text-3xl font-bold ${
                  availability ? "text-white" : "text-gray-800"
                }`}
              >
                Session Requests
              </h2>
              ...
              <ul className="space-y-3">
                {sessionRequests.map((request) => (
                  <li
                    ...
                    className={`rounded-xl p-3 flex justify-between items-center shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:translate-y-1 ${
                      availability ? "bg-gray-700" : "bg-gray-100"
                    }`}
                  >
                    <div>
                      <p
                        className={`text-lg font-semibold ${
                          availability ? "text-gray-100" : "text-gray-800"
                        }`}
                      >
                        {request.name}
                      </p>
                      <p
                        className={`text-sm ${
                          availability ? "text-gray-400" : "text-gray-500"
                        }`}
                      >
                        {request.type || "Live Assistance"}
                      </p>
                    </div>
                    <div className="flex items-center">
                      <FaClock
                        className={`mr-2 ${
                          availability ? "text-gray-400" : "text-gray-600"
                        }`}
                      />
                      <span
                        className={`${
                    ...
                    <div className="flex space-x-4">
                      ...
                      <button
                        ...
                        } font-semibold shadow-lg transition-transform transform hover:scale-105`}
                      >
                        Accept
                      </button>
